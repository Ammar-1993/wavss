-- Clean Database Seed for WAVSS
-- Contains schema and static vulnerability reference data.
...
('secheaders', 'Missing Security Headers', 'The web server does not return one or more modern security headers (such as Content-Security-Policy, X-Frame-Options, Strict-Transport-Security, or X-Content-Type-Options). These headers provide critical protection against common attacks like Cross-Site Scripting (XSS), clickjacking, and MIME-sniffing.', 'Configure the web server or application framework to consistently return strict security headers on all HTTP responses.', 'Medium', 12),
('fileexposure', 'Sensitive File Exposure', 'The application publicly exposes sensitive files such as version control directories (.git), environment configuration files (.env), database backups, or operating system metadata. These files often contain hardcoded credentials, API keys, or infrastructure details that can lead to complete server compromise.', 'Remove all sensitive files from the public web root. Ensure your web server configuration correctly denies access to dotfiles (like .env and .git) and backup file extensions.', 'Critical', 13);
