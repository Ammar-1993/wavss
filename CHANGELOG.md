...
### 26. Exclusion of Time-Based/Blind SQLi Module
* **Decision:** Decided to intentionally exclude a Time-Based (Blind) SQL Injection detection module from the manual development scope.
* **Reasoning:** Implementing this would require generating functional exploitation payloads (e.g., database-specific `SLEEP()` commands) that execute arbitrary delays on the target, crossing the line from defensive analysis into active functional exploitation. The team is referred to academically documented open-source tools (like sqlmap) for analyzing time-based vulnerabilities in a dedicated environment.

## Phase Five: Architecture Finalization & Core Modules

### 27. True Background Job Queue
* **Before:** Asynchronous scanning relied on a fragile 1-second timeout `curl` hack hitting the local webserver to run `begin_scan.php` in the background, offering no visibility, retries, or error tracking.
* **After:** Engineered a robust, database-backed job queue. Scans are enqueued via prepared SQL statements from the web UI, REST API, and Cron scripts. A dedicated, long-running CLI worker daemon (`scripts/worker.php`) continuously polls the queue, managing states (pending, running, done, failed) and cleanly capturing exceptions.

### 28. Security Headers Detection
* **Before:** The scanner did not evaluate modern HTTP security posture.
* **After:** Added `testSecurityHeaders.php` to actively audit responses for the presence of `Content-Security-Policy`, `X-Frame-Options`, `X-Content-Type-Options`, and `Strict-Transport-Security` (over HTTPS), accurately injecting findings into the report pipeline.

### 29. Sensitive File Exposure Detection
* **Before:** The scanner lacked checks for unintentionally exposed infrastructure files.
* **After:** Built `testSensitiveFileExposure.php` to securely iterate over high-risk paths (`.git/HEAD`, `.env`, backups, `.DS_Store`). It employs strict content signature validation to completely eliminate false positives generated by "soft 404" catch-all pages.
